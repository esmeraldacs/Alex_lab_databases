...
# Step 4: Extract raster values within the buffer
#pmp_meta_sf$HFP2009_values <- extract(footprint_raster_3395, pmp_meta_sf$buffer, 
                                      # fun = function(x, ...) {
                                          # if (length(x) > 0) {
                                             #  x <- na.omit(x) # Remove NA values before processing
...
#saveRDS(pmp_meta_sf, "Output_data/pmp_meta_sf.rds")

#pmp_meta_sf <- readRDS("Output_data/pmp_meta_sf.rds")
...
pmp_meta_df <- pmp_meta_sf %>%
  mutate(
    Pressure_avg12km = as.numeric(HFP2009_avg), # Convert matrix to numeric
...
ggplot() + 
  ...
  geom_point(data = pmp_meta_df, 
             aes(x = longitude, y = latitude, fill = Pressure_avg12km, color = Pressure_avg12km), size=0.5) +
...
```{r}
coordinates(pmp_meta) <- ~lon+lat
proj4string(pmp_meta) <- raster::CRS("+proj=longlat +datum=WGS84")

# Create 12 km buffers around each point
buffer_distance <- 12000  # 12 km in meters
buffers <- gBuffer(pmp_meta, width = buffer_distance, byid = TRUE)

# Plot the points and their corresponding buffers
#plot(buffers, col = rgb(0, 0, 1, 0.3), main = "Buffers Around Points", xlab = "Longitude", ylab = "Latitude")
#points(pmp_meta, col = "red", pch = 16, cex = 1.5)  # Plot the points on top

# Optionally, use ggplot2 for a more customized plot
#library(ggplot2)
# Convert buffers to a data frame for ggplot
#buffers_df <- fortify(buffers)

ggplot() +
  geom_polygon(data = fortify(buffers), aes(x = long, y = lat, group = group), fill = "blue", alpha = 0.3) +
  geom_point(data = pmp_meta@data, aes(x = lon, y = lat), color = "red", size = 3) +
  coord_fixed(ratio = 1) +
  labs(title = "Buffers Around Points", x = "Longitude", y = "Latitude")
```
